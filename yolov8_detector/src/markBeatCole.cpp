...
public:
    zedYolo() : Node("zed_yolo")
    {
        ...
        this->declare_parameter("model_path", rclcpp::ParameterValue("weights/yolov8n-seg.onnx"));
...
private:
    ...
    void imageCallback(const sensor_msgs::msg::Image::SharedPtr msg)
    {
        ...
        cv::cvtColor(inputIm, inputIm, cv::COLOR_BGR2RGB);
        ...
        for (Detection detection : detections)
        {
            ...
            RCLCPP_INFO(get_logger(), "Class ID: %d, Rect: %d %d %d %d", detection.classId(), detection.bbox().x, detection.bbox().y, detection.bbox().width, detection.bbox().height);
        }
