...
public:
        ...
        : Node("tensorrt_wrapper")
    {
        ...
        declare_parameter("threshold", rclcpp::ParameterValue(0.9));
        double threshold = get_parameter("threshold").as_double();

        declare_parameter("iou", rclcpp::ParameterValue(0.9));
        double iou = get_parameter("iou").as_double();

        RCLCPP_WARN(get_logger(), "ENGINE FILE PATH: %s", engine_file.c_str());
        RCLCPP_WARN(get_logger(), "Threshold: %f", threshold);
        RCLCPP_WARN(get_logger(), "IOU: %f", iou);

        // init and load the detector
        infer = std::make_shared<yolov5::Detector>();
        infer->init();
        infer->loadEngine(engine_file);
        infer->setScoreThreshold(threshold);
        infer->setNmsThreshold(iou);
